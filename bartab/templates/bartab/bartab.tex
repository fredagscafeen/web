...
\begin{longtabu} to \linewidth{| X[2, l] | X[9, l] | X[6, l] | X[3, r] | X[24, l] |}
\hline
\textbf{Indsat} & \textbf{Navn} & \textbf{Email} & \textbf{Saldo} & \textbf{Køb} \hfill \\ \hline

	{% for user in users %}
& {{ user.name | latex_trunc }} & {{ user.email | default_if_none:'' | latex_trunc }} & \hfill {{ user.balance_str }} & {% if user.has_credit_hold %}\textbf{KREDITSTOP}{% endif %} \\ \hline
	{% endfor %}
\end{longtabu}
