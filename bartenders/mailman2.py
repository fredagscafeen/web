...
class Mailman:
    ...
    def _login(self):
        ...
        if r.status_code == 401:
            ...
            raise OperationError(
                f"Got unknown status {r.status_code} while trying to login"
            )
    ...
    def _request(self, request_fun, url, *args, **kwargs):
        ...
        while True:
            ...
            if title_el and title_el.text.endswith(
                self.NEED_AUTHENTICATION_TITLE_SUFFIX
            ):
                if not first_try:
    ...
    def _get_csrf_token(self, url):
        ...
        if not r.ok:
            raise OperationError(
                f"Got unknown status {r.status_code} while trying to get csrf token"
            )
    ...
    def _post_form(self, url, data):
        ...
        if not r.ok:
            raise OperationError(
                f"Got unknown status {r.status_code} while trying to post form"
            )
    ...
    def get_subscribers(self):
        ...
        if not r.ok:
            raise OperationError(
                f"Got unknown status {r.status_code} while trying to get list of subscribers"
            )
    ...
    def add_subscriptions(
        self, emails, invite=False, send_welcome=True, notify_owner=False
    ):
