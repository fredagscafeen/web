...
from udlejning.models import UdlejningTent


class Command(ReminderCommand):
    help = "Sends tent rental reminders to responsible board members"

    def get_next_events(self):
        return UdlejningTent.objects.filter(
    ...
    def email_subject(self, humanized_bartenders, event):
        return f"Du er ansvarlig for en udlejning af festteltet i denne uge!"
...
Du/I er ansvarlige for at leje festteltet ud til {event.whoReserved}, {event.dateFrom.astimezone().strftime("d. %-d/%-m, kl. %H:%M")}.
...
/snek"""
    ...
    def text_format(self, humanized_bartenders, event):
        url = urljoin(
            settings.SELF_URL,
            reverse("admin:udlejning_udlejningtent_change", args=(event.pk,)),
        )
