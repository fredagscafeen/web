...
from udlejning.models import UdlejningProjector


class Command(ReminderCommand):
    help = "Sends projector rental reminders to responsible board members"

    def get_next_events(self):
        return UdlejningProjector.objects.filter(
...
/snek"""
    ...
    def text_format(self, humanized_bartenders, event):
        url = urljoin(
            settings.SELF_URL,
            reverse("admin:udlejning_udlejningprojector_change", args=(event.pk,)),
        )
