...
class Command(ReminderCommand):
    ...
    def get_next_events(self):
        DAYS_OFFSET = 2  # Runs on saturday, so it should take next weeks shift.
        return BoardMemberDepositShift.objects.filter(
            end_date__gte=timezone.now() + datetime.timedelta(DAYS_OFFSET),
            end_date__lte=timezone.now() + datetime.timedelta(DAYS_OFFSET + 7),
        )
