...
class ApiTests(APITestCase):
	...
	def setUp(self):
		self.user = get_user_model().objects.create_user(username='yeaah', password=self.password)
	...
	def create_brewery(self, name):
		return self.client.post(reverse('breweries-list'), {
			'name': name
		})
	...
	def test_get_authentication_token(self):
		r = self.client.post(reverse('token-auth'), {
